...
@app.exception_handler(TimelineException)
async def timeline_exception_handler(request: Request, exc: TimelineException):
    """Handle custom Timeline exceptions."""
    logger.error(f"Timeline exception: {exc.message}", extra={"details": exc.details})
...
# 1. Timeout middleware (30 second timeout)

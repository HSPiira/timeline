...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_email_account_email_address'), 'email_account', ['email_address'], unique=False)
    op.create_index(op.f('ix_email_account_subject_id'), 'email_account', ['subject_id'], unique=False)
    op.create_index(op.f('ix_email_account_tenant_id'), 'email_account', ['tenant_id'], unique=False)
    op.alter_column('workflow', 'trigger_event_type',
               existing_type=sa.VARCHAR(),
               comment='Event type that triggers this workflow',
               existing_nullable=False)
    op.alter_column('workflow', 'trigger_conditions',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
               comment='Optional JSON conditions (JSONPath expressions)',
               existing_nullable=True)
    op.alter_column('workflow', 'actions',
               existing_type=postgresql.JSON(astext_type=sa.Text()),
               comment='Array of actions to execute [{type, params}, ...]',
               existing_nullable=False)
    op.alter_column('workflow', 'max_executions_per_day',
               existing_type=sa.INTEGER(),
               comment='Rate limit: max executions per day (null = unlimited)',
               existing_nullable=True)
    op.alter_column('workflow', 'execution_order',
               existing_type=sa.INTEGER(),
               comment='Execution priority (lower = earlier)',
               existing_nullable=False)
    op.alter_column('workflow_execution', 'triggered_by_event_id',
               existing_type=sa.VARCHAR(),
               comment='Event that triggered this workflow',
               existing_nullable=True)
    op.alter_column('workflow_execution', 'status',
               existing_type=sa.VARCHAR(),
               comment='pending | running | completed | failed',
               existing_nullable=False)
    op.alter_column('workflow_execution', 'actions_executed',
               existing_type=sa.INTEGER(),
               comment='Number of actions successfully executed',
               existing_nullable=False)
    op.alter_column('workflow_execution', 'actions_failed',
               existing_type=sa.INTEGER(),
               comment='Number of actions that failed',
               existing_nullable=False)
    op.alter_column('workflow_execution', 'execution_log',
               existing_type=postgresql.JSON(astext_type=sa.Text()),

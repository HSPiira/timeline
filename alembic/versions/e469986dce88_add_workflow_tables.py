...
def upgrade() -> None:
    ...
    op.create_index('ix_workflow_execution_triggered_by_event_id', 'workflow_execution', ['triggered_by_event_id'])
    op.create_index('ix_workflow_execution_triggered_by_subject_id', 'workflow_execution', ['triggered_by_subject_id'])
    ...
    op.alter_column('document', 'is_latest_version',
               existing_type=sa.BOOLEAN(),
               nullable=False)
    op.drop_index(op.f('ix_document_tenant_checksum'), table_name='document')
    op.create_index('ix_document_checksum_unique', 'document', ['tenant_id', 'checksum'], unique=True)
    op.create_index('ix_document_tenant_subject', 'document', ['tenant_id', 'subject_id'], unique=False)
    op.create_index('ux_document_versions', 'document', ['tenant_id', 'subject_id', 'parent_document_id', 'version'], unique=True)
...
def downgrade() -> None:
    ...
    op.create_foreign_key(op.f('user_role_assigned_by_fkey'), 'user_role', 'user', ['assigned_by'], ['id'], ondelete='SET NULL')
    op.alter_column('user_role', 'assigned_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               nullable=False,
               existing_server_default=sa.text('now()'))
    ...
    op.alter_column('permission', 'created_at',
               ...
               nullable=False,
               existing_server_default=sa.text('now()'))
    op.create_index(op.f('ix_event_schema_active'), 'event_schema', ['tenant_id', 'event_type', 'is_active'], unique=False)
    ...
    op.create_index(op.f('ix_document_tenant_checksum'), 'document', ['tenant_id', 'checksum'], unique=False)
    op.alter_column('document', 'is_latest_version',
               existing_type=sa.BOOLEAN(),
               nullable=True)
    op.alter_column('document', 'file_size',
